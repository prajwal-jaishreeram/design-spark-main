...
const Login = () => {
  ...
  const { login, pendingPrompt } = useAuth();
      ...
      >
        <div className="text-center mb-8">
          <Link to="/" className="inline-flex items-center gap-2 mb-6">
            <img src={logo} alt="DesignForge" className="h-8 w-8" />
            <span className="font-bold text-xs uppercase tracking-widest">DesignForge</span>
          </Link>
          <h1 className="text-2xl font-bold uppercase tracking-wider mb-2">Welcome Back</h1>
          <p className="text-xs text-muted-foreground uppercase tracking-widest">Sign in to your account</p>
        </div>

        <div className="rounded-lg border border-border bg-card p-6">
          <Button variant="outline" className="w-full mb-4 h-11 text-xs uppercase tracking-wider">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              ...
              <Input id="email" type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="you@example.com" className="mt-1.5 h-11 bg-background" />
            </div>
            <div>
              <Label htmlFor="password" className="text-xs uppercase tracking-wider">Password</Label>
              <Input id="password" type="password" value={password} onChange={(e) => setPassword(e.target.value)} placeholder="••••••••" className="mt-1.5 h-11 bg-background" />
            </div>
            <Button className="w-full h-11 text-xs uppercase tracking-wider" type="submit">
