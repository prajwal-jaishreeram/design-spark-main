...
const Login = () => {
  ...
  const { signIn, signInWithGoogle, pendingPrompt, isAuthenticated, loading } = useAuth();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (error) {
      ...
      if (error.message?.includes("Invalid login credentials")) {
        description = "Email or password is incorrect. Please check and try again, or sign up for a new account.";
  ...
  const handleGoogleLogin = async () => {
    try {
      ...
      if (error) {
        // If provider not enabled, show helpful message
        ...
        <div className="rounded-lg border border-border bg-card p-6">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <Button className="w-full h-11 text-xs uppercase tracking-wider" type="submit" disabled={isLoading}>
              {isLoading ? <Loader2 className="h-4 w-4 animate-spin" /> : <>Sign In <ArrowRight className="h-3.5 w-3.5 ml-1" /></>}
            </Button>
            <div className="text-right">
              <Link to="/reset-password" className="text-[10px] text-muted-foreground hover:text-foreground transition-colors uppercase tracking-wider">
                Forgot password?
              </Link>
            </div>
          </form>
